#include "OrientationManager.h"
  ...
  // Checks if the orientation has been stable for more than 3 seconds and updates display
  void OrientationManager::checkStabilityAndUpdateDisplay() {
    if (orientationChanged && millis() - lastUpdateTime > 3000) {
      if(stableOrientation != currentOrientation){
        ...
        display.println("upper side is:");
        display.print(stableOrientation, 1);
        display.println("");
        display.display();
      }
