...
{
  lastConnectedTimeA=data.intData();
  Serial.println(lastConnectedTimeA);
}
...
void init_wifi() {  //connect to Wifi
  ...
  res = wifiManager.autoConnect("ConfigCubeB","password");
  if(!res) {
...
Serial.println("fb");
  ...
  if (!Firebase.RTDB.beginStream(&streamConnection, "/cube_a/last_connected"))
  ...
  if (!Firebase.RTDB.beginStream(&streamState, "/cube_a/state"))
...
void init_state(){
  if (Firebase.ready()){
      if (Firebase.RTDB.getInt(&fbdo, "cube_b/state")) {
        state = fbdo.to<int>();
      } else {
...
bool isBoardAConnected() {
  ...
  if (lastConnectedTimeA == 0 || currentTime == 0) {
    return false;
  } else 
  return currentTime - lastConnectedTimeA < 15;
}
...
void loop() {
    ...
    if (Firebase.ready() && signupOK && millis() - sendDataPrevMillis > 1000 || sendDataPrevMillis == 0){
      ...
      if (Firebase.RTDB.setInt(&fbdo, "cube_b/last_connected", getTime())) {
      } else {
    ...
    if(orientationManager.checkStabilityAndUpdate()){
      ...
      if(orientationManager.get_stable_state()!=2&&orientationManager.get_stable_state()!=4){
        ...
        if (Firebase.RTDB.setInt(&fbdo, "cube_b/state", state)) {
        // do nothing
        } else {
        ...
        if (Firebase.RTDB.pushJSON(&fbdo, "/cube_b/state_and_time", &timeAndState)) {
            // do nothing
          } else {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        display_led(1);
      res = wifiManager.autoConnect("ConfigCubeB","password"); // This will open the configuration portal if not able to reconnect.
    } else {
      if (!Firebase.ready()) {
          write_to_display("Fb error",state);
          display_led(1);
      }else if(!isBoardAConnected()){
          write_to_display("  A is offline",state);
          display_led(2);
      }
       else {
