...
void init_wifi() {  //connect to Wifi
  ...
  res = wifiManager.autoConnect("ConfigBoardB","password");
  if(!res) {
...
void init_firebase() {  // connect to firebase
  ...
  if (!Firebase.RTDB.beginStream(&streamConnection, "/board_a/last_connected"))
  ...
  if (!Firebase.RTDB.beginStream(&streamPresses, "/board_a/total_presses"))
...
void loop() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
      ...
      u8g2.sendBuffer();
      res = wifiManager.autoConnect("ConfigBoardB","password"); // This will open the configuration portal if not able to reconnect.
      if(res){
        u8g2.clearBuffer();
        u8g2.drawStr(0, 40, "connected");
      }
